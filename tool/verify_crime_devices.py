#!/usr/bin/env python3
"""Drive the real production Crime routes on Owner A Android and Owner B emulator.
...
EVIDENCE = ROOT / 'build/crime-wave5-evidence'
PACKAGE = 'com.locatemy.crimeqa'
        ...
        with lock:
            for connection in tuple(active):
                try:
                    ...
                    pass
try:
    with tempfile.TemporaryDirectory(prefix='locatemy-crime-device-') as temporary:
        ...
        print('Building isolated real production Crime QA app', flush=True)
        result = subprocess.run(['flutter', 'build', 'apk', '--debug', '--target', 'tool/crime_security_device.dart', '--dart-define=LOCATEMY_CRIME_FIXTURE_PORT=' + port], cwd=build, capture_output=True, text=True)
        ...
        target = EVIDENCE / 'crime-device-debug.apk'
    ...
    for device in args.devices:
        ...
        try:
            ...
            capture('single-zh')
            tap('暴力犯罪 (2,953)')
            capture('assault-filter-zh')
            tap('具体犯罪类型 ▾')
            tap('谋杀 (67)')
            capture('type-filter-zh')
            tap('全部 (13,739)')
            offline(True)
            tap('重试 / 刷新')
            top()
            ...
            tap('交换 A/B')
            capture('comparison-swapped-zh')
            back()
            back()
            tap('语言')
            tap('Single')
            tap('View full analysis')
            tap('Crime and security')
            wait('13,739')
            capture('single-en')
            ...
            if any(message in logs for message in ['A RenderFlex overflowed', 'EXCEPTION CAUGHT BY', 'CRIME_DEVICE: FAILED']):
                raise RuntimeError(tag + ': Flutter error; sanitized log retained')
            (EVIDENCE / (tag + '-verification.json')).write_text(json.dumps({**stamp, 'device': tag, 'single': 'PASS', 'filters': 'PASS', 'offline_cached': 'PASS', 'retry': 'PASS', 'map_return': 'PASS', 'comparison_swap': 'PASS', 'bilingual': 'PASS', 'font_200': 'PASS', 'logout': 'PASS'}, indent=2) + '\n')
