#!/usr/bin/env python3
"""Drive the real production Socio routes on Owner A Android and Owner B emulator.
...
EVIDENCE = ROOT / 'build/socio-wave6-evidence'
PACKAGE = 'com.locatemy.socioqa'
        ...
        with lock:
            for connection in tuple(active):
                try:
                    ...
                    pass
try:
    with tempfile.TemporaryDirectory(prefix='locatemy-socio-device-') as temporary:
        ...
        print('Building isolated real production Socio QA app', flush=True)
        result = subprocess.run(['flutter', 'build', 'apk', '--debug', '--target', 'tool/socio_economic_device.dart', '--dart-define=LOCATEMY_SOCIO_FIXTURE_PORT=' + port], cwd=build, capture_output=True, text=True)
        ...
        target = EVIDENCE / 'socio-device-debug.apk'
