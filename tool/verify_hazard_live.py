...
try:
    ...
    env={**os.environ,**{k:v for k,v in values.items() if k!='SUPABASE_SECRET_KEY'},'LOCATEMY_OTHER_EMAIL':other_email,'LOCATEMY_OTHER_PASSWORD':other_password,'LOCATEMY_HAZARD_FIXTURE':name,'LOCATEMY_HAZARD_LIVE':'1'}
    result=subprocess.run(['flutter','test','test/live/hazard_reporting_live_test.dart','--reporter','expanded'],env=env,capture_output=True,text=True)
    ...
    request('/rest/v1/crowdsourced_hazards?title=like.'+urllib.parse.quote(name+'%'),'DELETE')
