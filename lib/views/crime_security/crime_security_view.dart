      ...
      body: showError
        ...
        child: Column(
          ...
          children: [
            ...
            _SafetyMapCard(
              latLng: latLng,
              districtName: securityData['district_name']?.toString() ?? '',
              securityScore:
                  (securityData['security_score'] as num?)?.toDouble(),
              onOpenMainMap: () {
...
class _SafetyMapCardState extends State<_SafetyMapCard> {
  ...
  Future<void> _loadBoundary() async {
    ...
    final boundary =
        await SecurityRepository().fetchPoliceDistrictBoundary(widget.districtName);
    if (!mounted) return;
  ...
  (Color, Color) _boundaryColors(double? score) {
    if (score == null) return (AppColors.info.withValues(alpha: 0.10), AppColors.info);
    if (score >= 8) {
  ...
  List<HazardMarker> _nearbyHazards(HazardProvider provider) {
            ...
            Row(
              children: [
                Container(
                  ...
                  child: Icon(hazard.type.icon,
                      color: hazard.type.color, size: 22),
                ),
...
List<Polygon<Object>> _geoJsonToPolygons(
  ...
  for (final polygonRaw in polygonsCoords) {
    ...
    final holes = rings.skip(1).map(toPoints).where((h) => h.length >= 3).toList();
    result.add(Polygon<Object>(
      points: outer,
