              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text('$originName → $destName：暂无生活开销数据',
                    ...
                    FilledButton.icon(
                      onPressed: () =>
                          budgetProvider.fetchComparison(originName, destName, force: true),
          ...
          children: [
                ...
                children: [
                  ...
                  Text(
                    l10n.lifestyleComparisonText(
                      destName,
                      (comparison['budget_diff_pct'] as num?)?.toStringAsFixed(1) ?? '0.0',
                      (comparison['is_improvement'] ?? false) ? l10n.less : l10n.more,
                      originName,
                    ),
            ...
            BentoCard(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(l10n.budgetTranslation,
                      style: Theme.of(context).textTheme.titleMedium),
                  const SizedBox(height: 12),
                  _buildCpiRow('${comparison['origin_district'] ?? originName} (${comparison['origin_state'] ?? '-'})',
                      comparison['origin_cpi'], Icons.location_on_rounded, AppColors.primaryBase),
                  const Divider(height: 20),
                  _buildCpiRow('${comparison['target_district'] ?? destName} (${comparison['target_state'] ?? '-'})',
                      comparison['target_cpi'], Icons.flag_rounded, AppColors.accent),
                  const Divider(height: 20),
                  if ((comparison['base_budget'] as num?) != null)
                    Text(
            ...
            _PriceComparisonChart(
              originPrices: comparison['origin_prices'] as List? ?? const [],
              targetPrices: comparison['micro_prices'] as List? ?? const [],
                    ...
                    bottomTitles: AxisTitles(
                      sideTitles: SideTitles(
                        ...
                        getTitlesWidget: (value, meta) {
                          ...
                          final short = name.length > 12 ? '${name.substring(0, 11)}…' : name;
                          return Padding(
                    ...
                    for (var i = 0; i < rows.length; i++)
                      BarChartGroupData(x: i, barsSpace: 3, barRods: [
                        BarChartRodData(
                          toY: rows[i].origin,
                          width: 8,
                          color: AppColors.primaryBase.withValues(alpha: 0.55),
    ...
    return Column(
      ...
      children: [
        TextField(
          ...
          style: const TextStyle(
              fontSize: 20, fontWeight: FontWeight.bold, color: AppColors.primaryBase),
        ),
        const SizedBox(height: 8),
        Text(
          equivalent != null
