...
class _PropertyArchiveScreenState extends State<PropertyArchiveScreen> {
  ...
  void _compareSelected() {
    if (_selectedIds.length < 2) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select at least 2 properties to compare')),
      );
      return;
    }
    
    final inspections = Provider.of<PropertyProvider>(context, listen: false).inspections;
    final selectedInspections = inspections.where((i) => _selectedIds.contains(i.id)).toList();

    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => PropertyComparisonScreen(inspections: selectedInspections),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final inspections = propertyProvider.inspections;

    return Scaffold(
      backgroundColor: AppColors.backgroundLight,
      appBar: AppBar(
        title: Text(_isSelectionMode ? '${_selectedIds.length} Selected' : 'Property Archive'),
          ...
          if (!_isSelectionMode) ...[
            ...
            if (inspections.isNotEmpty)
              TextButton(
                ...
                child: const Text('Compare', style: TextStyle(color: AppColors.primaryBase)),
              ),
          ],
          if (_isSelectionMode)
            IconButton(
              icon: const Icon(Icons.compare_arrows, color: AppColors.primaryBase),
  ...
  Widget _buildEmptyState(AppLocalizations l10n) {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.inventory_2_outlined, size: 80, color: AppColors.textMutedDark.withOpacity(0.3)),
  ...
  Widget _buildPropertyCard(PropertyInspection inspection, AppLocalizations l10n) {
    ...
    return Padding(
      ...
      child: Dismissible(
        ...
        onDismissed: (_) {
          Provider.of<PropertyProvider>(context, listen: false).deleteInspection(inspection.id);
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Moved to recycle bin')),
          );
        },
        child: GestureDetector(
          onTap: _isSelectionMode ? () => _toggleSelection(inspection.id) : () => _navigateToDetail(inspection),
          child: BentoCard(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: inspection.photos.isEmpty
                        ? const Icon(Icons.home_work_outlined, color: AppColors.textMutedDark, size: 40)
                        : null,
                  ),
                  // Info
                  Expanded(
