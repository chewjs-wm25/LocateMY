...
class _AddPropertyScreenState extends State<AddPropertyScreen> {
  ...
  void initState() {
    ...
    if (widget.existingInspection != null) {
      ...
      _priceController = TextEditingController(text: inspection.price.toString());
      ...
      if (inspection.latitude != null && inspection.longitude != null) {
        try {
          _selectedSavedLocation = locationProvider.savedLocations.firstWhere((loc) => 
      ...
      _nameController = TextEditingController(text: propertyProvider.draftName);
      _priceController = TextEditingController(text: propertyProvider.draftPrice);
      _notesController = TextEditingController();
      _monsoonChecklist = Map.from(propertyProvider.draftMonsoonChecklist);
      _selectedPhotos = []; // In a real app, we might draft photos too
      
      if (propertyProvider.draftSavedLocationId != null) {
        try {
          _selectedSavedLocation = locationProvider.savedLocations.firstWhere(
            (loc) => loc.id == propertyProvider.draftSavedLocationId
          );
        } catch (_) {}
      }
      ...
      _nameController.addListener(() {
        propertyProvider.updateDraft(name: _nameController.text);
      });
      _priceController.addListener(() {
        propertyProvider.updateDraft(price: _priceController.text);
      });
    }

    // Auto-select logic if draft is empty but map has a selection
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (widget.existingInspection == null && _selectedSavedLocation == null && locationProvider.selectedLocation != null) {
        try {
          final matched = locationProvider.savedLocations.firstWhere((loc) => 
            (loc.location.latitude - locationProvider.selectedLocation!.latitude).abs() < 0.0001 &&
            (loc.location.longitude - locationProvider.selectedLocation!.longitude).abs() < 0.0001
          );
          setState(() {
            _selectedSavedLocation = matched;
            propertyProvider.updateDraft(savedLocationId: matched.id);
    ...
    if (_formKey.currentState!.validate() && _selectedSavedLocation != null) {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(widget.existingInspection != null ? 'Property updated successfully' : 'Property added successfully')),
        );
      }
    } else if (_selectedSavedLocation == null) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _goToMapToPick() {
    ...
    Provider.of<LocationProvider>(context, listen: false).setAnalysisReportOpen(false);
  ...
  Widget build(BuildContext context) {
    ...
    final propertyProvider = Provider.of<PropertyProvider>(context); // Listen for draft/loading state changes
    final savedLocations = locationProvider.savedLocations;

    return Scaffold(
      appBar: AppBar(
        title: Text(widget.existingInspection != null ? 'Edit Property' : l10n.addProperty),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _isSubmitting 
                    ...
                    : Text(
                        widget.existingInspection != null ? 'Update Property' : l10n.saveProperty, 
  ...
  Widget _buildLocationSelector(AppLocalizations l10n, List<SavedLocation> savedLocations) {
    return BentoCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
            ...
            onChanged: (val) {
              setState(() {
                ...
                if (val != null) {
                  ...
                  Provider.of<PropertyProvider>(context, listen: false).updateDraft(savedLocationId: val.id);
                }
              });
            },
          ),
    ...
    return BentoCard(
      ...
      child: ClipRRect(
        ...
        child: FlutterMap(
          ...
          options: MapOptions(
            initialCenter: _selectedSavedLocation?.location ?? const LatLng(3.1390, 101.6869),
            initialZoom: 14,
  ...
  Widget _buildRiskAssessmentBento(AppLocalizations l10n) {
    ...
    final bool isSentul = _selectedSavedLocation?.name.contains('Sentul') ?? false;
    ...
    return Column(
      children: [
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  Text(
                    isSentul ? '7.8' : '8.5', 
                    style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: isSentul ? AppColors.warning : AppColors.success)
                  ),
                  StatusBadge(
        ...
        BentoCard(
          child: Row(
            children: [
              const Icon(Icons.local_police_outlined, color: AppColors.primaryBase),
              const SizedBox(width: 12),
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(l10n.policeDistrict, style: const TextStyle(fontSize: 12, color: AppColors.textSecondaryLight)),
                  Text(
              ...
              StatusBadge(
                ...
                icon: isSentul ? Icons.warning_amber_rounded : Icons.check_circle_outline
              ),
            ],
          ),
        ),
      ...
      child: Column(
        children: [
          ...
          StarRatingSelector(
            ...
            onRatingChanged: (val) => _updateChecklist('waterproofing_score', val),
          ),
          const Divider(),
          StarRatingSelector(
            label: l10n.humidity,
            ...
            Row(
              ...
              children: [
                ...
                Text(
                  ...
                  style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: AppColors.primaryBase),
                ),
              ],
            ),
            const SizedBox(height: 8),
                          ...
                          GestureDetector(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                border: isMain ? Border.all(color: AppColors.primaryBase, width: 2) : null,
                              ),
                              child: const Icon(Icons.image, color: AppColors.textMutedDark),
                            ),
                          ),
                          if (isMain)
                            Positioned(
                            ...
                            child: GestureDetector(
                              onTap: () {
                                setState(() {
                                  ...
                                  if (_mainPhotoIndex >= _selectedPhotos.length) {
                                    _mainPhotoIndex = 0;
                                  }
                                });
                              },
            ...
            if (_selectedPhotos.length > 3)
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    const Icon(Icons.swipe_left_rounded, size: 14, color: AppColors.textMutedDark),
          ...
          Center(
            child: TextButton.icon(
              onPressed: () {
                // Mock adding a photo
                setState(() {
                  _selectedPhotos.add('mock_photo_${_selectedPhotos.length + 1}');
  ...
  Widget _buildFloodHistoryToggle() {
    return BentoCard(
      ...
      child: SwitchListTile(
        title: const Text('Local Flood Signs Observed', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500)),
        subtitle: const Text('e.g. water marks on walls, neighbor reports', style: TextStyle(fontSize: 12)),
