...
class HomeRepository {
  ...
  Future<Map<String, dynamic>> _getIncomeStats() async {
    try {
      ...
      if (latestDateResp.isEmpty) {
        debugPrint(
          'HomeRepository: No income data found in hh_income_district',
        );
        return {'median': null, 'trend': null};
      }

      final latestDate = latestDateResp.first['date'];
      final latestData = await _supabase
          .from('hh_income_district')
      ...
      final latestAvg =
          latestData.fold<num>(0, (p, e) => p + (e['income_median'] as num)) /
          latestData.length;

      final prevDateResp = await _supabase
          .from('hh_income_district')
          .select('date')
          .lt('date', latestDate)
          .order('date', ascending: false)
          .limit(1);
      if (prevDateResp.isEmpty)
        return {'median': latestAvg.toInt(), 'trend': null};
      ...
      final prevAvg =
          prevData.fold<num>(0, (p, e) => p + (e['income_median'] as num)) /
          prevData.length;
  ...
  Future<double?> _getInflationRate() async {
    try {
      ...
      var latestData = await _supabase
          .from('cpi_state')
          .select('index')
          .eq('date', latestDate)
          .ilike('division', '00%');
      ...
      final latestAvg =
          latestList.fold<num>(0, (p, e) => p + (e['index'] as num)) /
          latestList.length;

      final oneYearAgo = DateTime.parse(
        latestDate,
      ).subtract(const Duration(days: 365));
      final formattedOneYearAgo =
          "${oneYearAgo.year}-${oneYearAgo.month.toString().padLeft(2, '0')}-01";

      var prevData = await _supabase
          .from('cpi_state')
          .select('index')
      ...
      if (prevData == null || (prevData as List).isEmpty) {
        debugPrint(
          'HomeRepository: No historical CPI data found, attempting to get any previous data',
        );
        prevData = await _supabase
            .from('cpi_state')
            .select('index')
            .lt('date', latestDate)
            .order('date', ascending: false)
      ...
      if (prevData == null || (prevData as List).isEmpty) {
        debugPrint(
          'HomeRepository: No historical CPI data found to calculate inflation',
        );
        return null;
      }
      final prevList = prevData as List;
      final prevAvg =
          prevList.fold<num>(0, (p, e) => p + (e['index'] as num)) /
          prevList.length;
  ...
  Future<double?> _getNationalAverageICI() async {
    try {
      final data = await _supabase
          ...
          .select('piped_water, electricity, sanitation')
          .order('date', ascending: false)
          .limit(16);
      if (data.isEmpty) {
        debugPrint(
          'HomeRepository: No amenities data found in hh_access_amenities',
        );
        return null;
      }
  ...
  Future<double?> _getNationalCrimeTrend() async {
    try {
      ...
      final latestCount = await _supabase
          .from('crime_stats')
          .select('crimes')
          .eq('date', latestDate);
      final latestTotal = latestCount.fold<num>(
        0,
        (p, e) => p + (e['crimes'] as num),
      );
      ...
      final prevCount = await _supabase
          .from('crime_stats')
          .select('crimes')
          .eq('date', prevDate);
      final prevTotal = prevCount.fold<num>(
        0,
        (p, e) => p + (e['crimes'] as num),
      );
