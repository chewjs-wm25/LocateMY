...
class _NearbyFacilitiesViewState extends State<NearbyFacilitiesView> {
  ...
  void didChangeDependencies() {
    ...
    final location = context.watch<LocationViewModel>().selectedLocation;
    final provider = context.read<NearbyFacilitiesViewModel>();
    ...
    if (provider.requestKey != key) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) return;
        context.read<NearbyFacilitiesViewModel>().loadNearbyFacilities(
          location,
        );
      });
    }
  ...
  Widget build(BuildContext context) {
    ...
    final provider = context.watch<NearbyFacilitiesViewModel>();
  ...
  Widget _buildContent(
    NearbyFacilitiesViewModel provider,
    ...
    final coveredCategories = <FacilityCategory>{
      for (final f in facilities) f.category,
    };
        ...
        children: [
          ...
          _buildCategorySection(
            ...
            facilities: facilities
                .where((f) => f.category == FacilityCategory.healthcare)
                .toList(),
          ),
          const SizedBox(height: 16),
          _buildCategorySection(
            ...
            facilities: facilities
                .where((f) => f.category == FacilityCategory.education)
                .toList(),
          ),
          const SizedBox(height: 16),
          _buildCategorySection(
            ...
            facilities: facilities
                .where((f) => f.category == FacilityCategory.living)
                .toList(),
          ),
          const SizedBox(height: 16),
          _buildCategorySection(
            ...
            facilities: facilities
                .where((f) => f.category == FacilityCategory.transport)
                .toList(),
          ),
          const SizedBox(height: 16),
          _buildCategorySection(
            ...
            facilities: facilities
                .where((f) => f.category == FacilityCategory.safety)
                .toList(),
          ),
          const SizedBox(height: 16),
          _buildCategorySection(
            ...
            facilities: facilities
                .where((f) => f.category == FacilityCategory.leisure)
                .toList(),
          ),
          const SizedBox(height: 16),
          _buildCategorySection(
            ...
            color: Colors.brown,
            facilities: facilities
                .where((f) => f.category == FacilityCategory.risk)
                .toList(),
          ),
  ...
  Widget _buildError(NearbyFacilitiesViewModel provider, LatLng location) {
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            FilledButton.icon(
              onPressed: () => context
                  .read<NearbyFacilitiesViewModel>()
  ...
  Widget _buildEmpty(NearbyFacilitiesViewModel provider, LatLng location) {
    return Center(
      child: Padding(
        padding: const EdgeInsets.all(24),
        child: Column(
  ...
  Widget _buildCategorySection({
    ...
    final summaryLines = <String>[
      if (nearest != null) '最近：${nearest.name} · ${nearest.distance.toInt()}m',
