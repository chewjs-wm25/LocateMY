...
class _TransportationViewState extends State<TransportationView> {
  ...
  Widget build(BuildContext context) {
    ...
    final transitProvider = Provider.of<TransitViewModel>(context);
    ...
    final hasDataForKey =
        transitProvider.requestKey == key && transitProvider.hasLoaded;
    final showError =
        !isLoading &&
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            FilledButton.icon(
              onPressed: () => provider.loadNearbyStops(
                latLng.latitude,
                latLng.longitude,
                force: true,
              ),
          ...
          child: stops.isEmpty
              ...
              : Column(
                  children: List.generate(stops.length, (index) {
                    ...
                    final isBus = (stop['transit_type']?.toString() ?? '')
                        .toLowerCase()
                        .contains('bus');
        ...
        Expanded(
          child: Text(
            _showStops && stops.isNotEmpty
                ...
                : (stops.isEmpty ? '1.5km 范围内暂无已收录站点' : '已隐藏站点图层，点击右上角图标恢复'),
            style: const TextStyle(
              fontSize: 11,
              color: AppColors.textMutedLight,
            ),
