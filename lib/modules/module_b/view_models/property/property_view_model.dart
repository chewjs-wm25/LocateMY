...
import 'package:locate_my/modules/module_b/repositories/property/security_risk_repository.dart';
...
class PropertyViewModel extends ChangeNotifier {
  ...
  void updateDraft({
    ...
    if (monsoonChecklist != null)
      _draftMonsoonChecklist = Map.from(monsoonChecklist);
    notifyListeners();
  }
  ...
  Future<void> addInspection({
    ...
    if (latitude != null && longitude != null) {
      final riskData = await _riskRepository.getRiskContext(
        latitude,
        longitude,
      );
  ...
  Future<void> updateInspection(
    ...
    if (index != -1) {
      ...
      if (latitude != null &&
          longitude != null &&
          (latitude != _inspections[index].latitude ||
              longitude != _inspections[index].longitude)) {
        _isLoadingRiskData = true;
        notifyListeners();
