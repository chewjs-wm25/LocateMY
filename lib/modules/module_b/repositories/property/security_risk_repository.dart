...
class SecurityRiskRepository {
  ...
  Future<Map<String, dynamic>> getRiskContext(double lat, double lng) async {
    ...
    final districtMatch = await _supabase.rpc(
      'match_police_district',
      params: {'lat': lat, 'lng': lng},
    );
    final districtInfo = (districtMatch as List).isNotEmpty
        ? districtMatch.first
        : {'name': 'Kuala Lumpur', 'id': 'PD-KL'};
    final districtName = districtInfo['name'];

    // 2. Fetch raw crime stats and user-generated hazards (对接真实表)
    final crimeStatsList = await _supabase
        .from('crime_stats')
        .select()
        .eq('district', districtName);
    final hazards = await _supabase
    ...
    return {
      ...
      'crime_stats': {
        for (var e in crimeStatsList)
          e['category'] as String: e['crimes'] as int,
      },
    };
  }
}
