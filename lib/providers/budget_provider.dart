...
import '../repositories/cost_of_living_repository.dart';
...
class BudgetProvider extends ChangeNotifier {
  ...
  Future<void> _loadUserScenarios() async {
    ...
    try {
      final data = await _supabase.from('user_budget_scenarios')
          .select()
          .eq('user_id', user.id);
      
      _scenarios = (data as List).map((item) => BudgetScenario(
        id: item['id'].toString(),
        name: item['scenario_name'],
        housingWeight: (item['max_rent'] as num?)?.toDouble() ?? 0.0,
        foodWeight: (item['living_expenses'] as num?)?.toDouble() ?? 0.0,
        transportWeight: (item['transport_allowance'] as num?)?.toDouble() ?? 0.0,
  ...
  Future<void> addScenario(String name) async {
    ...
    try {
      final response = await _supabase.from('user_budget_scenarios').insert(insertData).select();
  ...
  Future<void> renameScenario(String id, String newName) async {
    try {
      await _supabase.from('user_budget_scenarios')
  ...
  Future<void> updateCurrentScenario({
    ...
    if (transportWeight != null) updateData['transport_allowance'] = transportWeight;
