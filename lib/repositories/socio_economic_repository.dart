...
class SocioEconomicRepository {
  ...
  Future<Map<String, dynamic>> getSocioEconomicData({
    ...
    final giniDistrictRows = await _safeRows(() => _supabase
        .from('hh_inequality_district')
        .select('state, district, date, gini')
        .eq('district', districtName)
        .order('date', ascending: false));

    final stateHies = await _safeRows(() => _supabase
        .from('hies_state')
        .select('date, income_mean, income_median, gini, poverty')
        .eq('state', stateName)
        .order('date', ascending: false));

    final nationalGiniRows = await _safeRows(() => _supabase
        .from('hh_inequality')
        .select('date, gini')
        .order('date', ascending: false));

    final nationalIncomeRows = await _safeRows(() => _supabase
        .from('hh_income')
        .select('date, income_mean, income_median')
        .order('date', ascending: false));

    final percentileRows = await _safeRows(() => _supabase
    ...
    final Map<String, dynamic>? stateHiesRow =
        stateHies.isNotEmpty ? (stateHies.first as Map).cast<String, dynamic>() : null;
    final Map<String, dynamic>? nationalGiniRow =
        nationalGiniRows.isNotEmpty ? (nationalGiniRows.first as Map).cast<String, dynamic>() : null;
    final Map<String, dynamic>? nationalIncomeRow =
        nationalIncomeRows.isNotEmpty ? (nationalIncomeRows.first as Map).cast<String, dynamic>() : null;

    final double? giniDistrict =
        (districtGiniRow?['gini'] as num?)?.toDouble();
    ...
    final pB40 = sigma == null ? null : _lognormalCdf(b40Ceiling, lnMedian, sigma);
    final pT20 = sigma == null ? null : 1 - _lognormalCdf(t20Floor, lnMedian, sigma);
    ...
    final data = <String, dynamic>{
      ...
      't20_floor': t20Floor,
      // 州级背景（hies_state）
      'state_median_income': (stateHiesRow?['income_median'] as num?)?.toDouble(),
      'state_poverty_rate': (stateHiesRow?['poverty'] as num?)?.toDouble(),
      // 全国背景（hh_income / hh_inequality）
      'national_median_income': (nationalIncomeRow?['income_median'] as num?)?.toDouble(),
      'national_mean_income': (nationalIncomeRow?['income_mean'] as num?)?.toDouble(),
  ...
  double _giniFromSigma(double sigma) => 2 * _normalCdf(sigma / math.sqrt(2)) - 1;
