...
本简报只报告公开一手页面截至 2026-09-14 所能核实的内容。批准的固定 DOSM 文件已在本地取得并通过其
SHA-256 核验；空间资料验证、修复策略和远端导入/读取状态只在
[`RISK-GEO-02`](../design/system/risks-and-decisions.md#risk-geo-02-资料决定与未关闭证据)维护。本简报不证明
该文件已经导入或能够通过 `GEO-002` 读取。
