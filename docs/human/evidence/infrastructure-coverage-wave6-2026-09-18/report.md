...
开发Supabase真实现况：schools_district有3977行，并非目录旧称完全缺失。Petaling的hospital_beds无记录；schools最新2025，teachers/enrolment最新2022；医疗因无记录缺失；教育分别取学校2025与教师/学生2022的有效聚合，不因统计年不同错误标缺失，不猜测地区或组成。2026-09-17真实交通成功时可用供水/供电/交通构成ICI；设备实际当前日期不保证GTFS日期范围内，允许如实Unavailable，不回退分析日期。没有导入新政府资料。

## 验证

- `flutter test test/features/infrastructure_coverage --reporter expanded`：23项通过。覆盖公开公式/原始统计、缺失/零/60%、全国百分位、人口年份、教育both-sex、canonical交通请求、权重preview/save失败重试/恢复、旧请求/dispose、SQLite有效期、刷新失败不续期/过期响应不覆盖、Adapter无权限/格式/范围与恢复、320px小屏200%Widget。
- `flutter test --reporter expanded`：全仓290项通过，12项显式live opt-in跳过；随后新增验证仅针对本Feature重复（最终Feature23项）。
...
4. 当前阻塞：无实现验收阻塞；独立 Luna High 审查仍待主Agent执行。当前真实政府医疗记录缺失是合法结果，不冒充服务覆盖或资料成功。
...
## Luna Spec 复审修复（基于17e7786）

三个阻塞分别用公开行为逐片red→green解除：

1. 教育各输入年份独立：2025学校+2022教师+2023学生的test红为教育null，绿色为100；全国学校密度锁学校年，师生资源分别锁该次选取的教师/学生年。latest aggregate按统计日期整组有效性选取，最新某一必要记录非法则整组回退，而不是去掉非法记录后对残余求和。非法医院最新组999/null回退2024完整组，已知零仍有效；教师最新组非法回退2022，学校2025/学生2023全国参照算例均得50。
2. 缓存异步交错：旧scope write已进入await时让新请求完成，red最终缓存tag为old而非new。每缓存key使用串行Future tail及请求revision：已in-flight旧写先结束，新写排在后面；过期尚未开始的写跳过。独立coordinate与共享district key均受保护，覆盖阻塞scope或coordinate写、同坐标或同district不同坐标四种交错，不通过仅write前一次generation检查冒充保证。
3. 真实年份保留：公开Coverage新增不可变sourceYears/populationYears，preview重算/summary/A-B不丢失；缺成员导致编译red，green保留医疗2024/人口2023。Widget红为无必要人口年份说明，green单点及A/B集中说明不同统计年份，学校/教师/学生分别保留并展示；同年统计合并为一项，仅人口与对应指标年不同再加人口说明。没有查询/缓存/TTL时间或逐行重复技术来源提示。
