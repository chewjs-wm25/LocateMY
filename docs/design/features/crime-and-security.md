...
未来唯一公开入口 `lib/features/crime_security/crime_security.dart`；使用具名安全分析服务，
显式输入合法地点／地理语境，提供州级可用／部分／不可用结果供页面与房产新增／坐标修改使用。
不要求 SHELL-001、PRIVACY-001、AccountScope 或个人化适配度消费者。

## 后续验收与开发前工作

B 固定实际声明及只读 RPC／来源 Adapter 后开发；不得把本契约称为已有完整可编译 declarations。
本模块测试：州级聚合、缺失／来源／年份、公式与 A/B；联合：Geo 正确口径、页面普通返回、
房产风险消费，B 主责、A 配合；最迟 Wave 5／6。Supabase schema/read RPC 尚 proposed，需真实 allow/deny 证据。
