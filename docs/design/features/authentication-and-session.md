...
| --- | --- | --- | --- |
| `AUTH-01` / `AT-AUTH-01`、`AT-OUT-02` | 当前设备无会话、有有效会话、凭据错误和会话不可用 | 启动或登录 | 认证入口、真实身份事实或可理解的失败恢复路径；不泄露凭据。 |
| `AUTH-02` / `AT-AUTH-02` | 注册后有会话、需要邮箱验证及资料登记失败 | 注册 | 明确区分认证结果和资料状态；需要验证时进入查收验证邮件状态；不显示虚假的验证状态。 |
| `AUTH-03` / `AT-AUTH-02` | 真实确认状态改变或暂时不可得 | 打开账户资料或恢复会话 | 仅据 Auth 呈现确认状态；未知有明确语义。 |
| `ACCOUNT-07` / `AT-OUT-01`、`AT-OUT-02`、`AT-SWITCH-01` | 取消退出、当前设备退出、退出或本机清理失败、随后登录另一账户 | 确认退出并换号 | 取消时保持当前会话；成功时回到登录页且不影响其他设备；任一阶段失败时不显示旧账户或新账户私有内容。 |
...
| 2026-09-14 | `Ready for Development` | 全面设计审查补齐 canonical 验收 ID；不改变可观察契约 | `AUTH-01`–`03`、`ACCOUNT-07`、`AT-AUTH-*`、`AT-OUT-*`、`AT-SWITCH-01` | 项目负责人（本次审查） |
