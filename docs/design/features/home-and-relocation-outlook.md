...
| --- | --- | --- | --- |
| `HOME-01` / `AT-HOME-01` | 全部三项主分项和收入资料可用；月度、季度、年度日期不同 | 打开首页 | 显示全国 `0–100` 搬家时机、状态、最多三条原因和每项真实数据日期；三项主分项及收入卡带单位/方向说明；不将异频日期或一次性导入写成实时统计 |
| `HOME-01`、`HOME-02` / `AT-HOME-01` | 任一主分项、收入资料、字段或历史样本缺失 | 打开或刷新首页 | 失败项显示带原因的 unavailable；任一主分项不可用则综合分不可用，其他有效卡保留；收入缺失不阻断主分项；未知不显示为零 |
| `HOME-01`、`HOME-02` / `AT-HOME-01` | 网络失败时有有效缓存、过期缓存或无缓存 | 打开或刷新首页 | 缓存结果分别标为 cached/stale 并显示观测日期/上次成功时间；无缓存显示可重试 unavailable；不得伪造 fresh 或样例值 |
| `HOME-03` / `AT-HOME-02` | 按钮与下拉的成功刷新、进行中重复触发、60 秒冷却与刷新失败 | 依次用两种入口刷新 | 两入口产生同一刷新结果；成功后显示剩余冷却秒数并拒绝重复强刷；失败不重置为成功且保留既有可用结果 |
| `HOME-01`–`HOME-03` / `AT-HOME-03` | opened、未认证、scope 关闭和探索地图 | 进入首页、关闭账户范围或选择探索地图 | 仅 opened 主应用读取/呈现 Home；关闭后旧结果不进入后续账户；探索地图只切换地图 Tab、不制造默认地点，Shell 拒绝时仍留在首页 |
...
| 2026-09-14 | `Ready for Development` | 全面设计审查补齐 `AT-HOME-01`–`03` 双向追踪；不改变可观察契约 | `HOME-01`–`HOME-03`、`FLOW-08` | 项目负责人（本次审查） |
