...
| `RISK-COST-01` | Cost 的地点指数、个人预案与临时换算口径若未固定，会生成不一致的预算和适配度输入 | `COST-01`–`03`、`ACCOUNT-09`、`SOCIO-02` | 项目负责人已固定核心市场指数、个人预案、临时换算、current 删除及 Q18 additive 的家庭月度总收入：月净收入只用于个人预算压力，家庭月度总收入只用于 Socio 收入位置，旧预案不猜测补齐；完整契约见 [Cost owning design](../features/cost-of-living-and-budget.md) | 影响复审后，migration 验证可空非负字段和旧预案保持缺失；Cost/Account/Socio 验证两种收入的分离、current/no-current、保存失败和换号 | Cost、Account Center、Socio Ready 前 | Q18 影响复审阻塞 Cost/Account 恢复 Ready；实现/集成验收继续阻塞 |
...
| `RISK-SCHEMA-02` | `user_ici_preferences` 现有五个 0–1 权重与三项 1–10 产品契约冲突 | Infrastructure、Account Privacy、Suitability | Schema Catalog 明确目标字段和旧表仅作迁移来源；远端只保存 last saved，single preview 仅页面内存；中性 ICI 不读账户权重且 A/B 固定中性 | 两账户迁移样本验证三项值、默认 5、旧 safety/amenity 不进入新对象，以及 preview 不跨账户/设备或进入 A/B | Infrastructure Ready 前 | Feature Ready 阻塞 |
