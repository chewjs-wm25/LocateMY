...
本表是跨 Feature Interface 摘要与外部 seam 的唯一真相。精确成员、领域类型、失败码、幂等键和 Adapter 规格留给 owning Feature/shared module 设计；数据字段只在 [Schema Catalog](../data/schema-catalog.md) 定义。表内无语言声明。

## 跨 Feature Interface

| ID | Owner | 消费者 | 用途 | 领域输入 | 领域输出与结果语义 | 权限 | 异步行为 | 副作用 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `AUTH-001` | Authentication & Session | Account Privacy；Application Shell；Account Center | 恢复/建立/结束当前设备会话并提供真实邮箱确认状态 | 启动恢复；规范化邮箱和本次密码；当前会话引用与 `current-device` 退出范围 | `authenticated`、`verification required`、`unauthenticated`、分类失败；账户引用、邮箱与真实确认状态；退出为本机会话已结束或分类失败 | 恢复/登录可匿名；资料与退出须匹配当前会话；凭据不越过 Interface | 单次动作返回最终分类；并发布可重复处理的会话变化事实 | 可更新或清除认证 SDK 本机状态；不影响其他设备或业务记录 |
| `PRIVACY-001` | Account Privacy | Application Shell；所有私有状态 Owner | 打开同账户 scope；退出/换号时关闭旧 scope 并证明本机私有状态已处理 | 已认证账户引用、原因、显式 Owner 清单及各 Owner 清理结果 | `opened`、`closed`、`identity mismatch` 或列出未完成 Owner 的分类失败；重复调用保持幂等结果 | 仅 Shell 发起；参与者只能处理输入账户分区 | 关闭等待所有 Owner；失败可重试且旧 scope 始终不可访问 | 打开匹配分区；关闭清除旧账户本机状态/队列/文件；不删远端记录、公共缓存或语言 |
...
| `GEO-001` | Geographic Context | Cost；Crime；Socio-economic；Infrastructure | 从坐标解析州、行政区或警区且保留边界资料版本 | 合法地点坐标与请求口径 | `resolved(context, boundary version)`、`unresolved(reason)`、`ambiguous(candidates)`；行政区和警区分开 | opened 主应用流程；边界资料为公共只读 | 单次查询；可缓存确定性结果，版本变化使旧缓存失效 | 只读边界资料/缓存；不改写 Map 地点或分析结果 |
...
| --- | --- | --- | --- | --- |
| `AUTH-002` | Authentication & Session | Supabase Auth | 邮箱密码注册/登录、会话恢复/结束和真实邮箱确认状态；SDK 网络/配置/权限失败必须分类 | 可更新当前设备 SDK 会话；测试 Adapter 无外部副作用 |
| `LOCATION-003` | Map / Location | Geoapify | 返回限于马来西亚的候选名称与坐标；候选仍须 `LOCATION-001` 做最终空间校验 | 防抖且忽略过期响应；只发网络读取 |
| `GEO-002` | Geographic Context | 版本化行政区/警区边界读取对象 | 给定坐标与口径返回零/一/多匹配及资料版本；不使用附近地区替代 | 可读公共缓存；测试 Adapter 使用固定边界样本 |
...
Supabase Data API、SQLite、文件和 Storage 是各 Owner 的 Data Adapter，不另建一个可绕过领域 Interface 的全局仓库。其可访问对象在 Schema Catalog 登记，精确 Adapter seam 在 owning design 固定。
...
每个 owning design 进入 `Ready for Development` 前补全其 Interface 的精确成员、稳定失败码、取消/过期响应规则、幂等键、性能界限、Adapter 和契约测试。消费者只有在所需上游 Interface 冻结后才能固定自己的调用；不能在消费者文档复制提供方契约。
