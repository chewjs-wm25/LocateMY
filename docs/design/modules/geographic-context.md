...
本文件协调统计地理语境与其消费者。地点仍由 Map / Location 以不可变合法地点引用提供；本模块只把其中坐标按所请求口径解释为州、行政区或警区，并使结果、资料来源和边界版本可被下游保留和解释。
...
- **拥有**：按行政统计或警察统计口径解析州、行政区、警区；返回每项的 resolved、unresolved 或 ambiguous 结果；随结果保留边界来源与版本；维护生产边界读取和确定性测试所共同使用的外部 seam。
- **不拥有**：马来西亚范围校验、地点搜索或可变选点（Map / Location）；地图图层；犯罪、成本、社会经济或基础设施读数；任何分析 Feature 的资料层级回退；边界资料的导入、迁移实现和下游政府数据查询策略。
- **删除测试**：若删除该 Module，四个消费者须各自处理行政区/警区的多边形版本、边界点和重叠、零匹配及其测试资料，且很容易把行政区误作警区。复杂性会回到所有调用方，故这是有 Leverage 和 Locality 的 shared module，而非转发层。
...
| Crime & Security | 取得犯罪统计和安全地图所用警区 | `GEO-001`；[治安事实源](../../knowledge_base/locatemy_product/features/crime_security.md) | 警区与版本，或明确未解析/歧义；警区绝不由行政区替代 |
...
| --- | --- | --- | --- | --- |
| `GEO-001` | Cost；Crime；Socio-economic；Infrastructure | 对一个不可变、已通过 `LOCATION-001` 校验的地点引用，取得所请求的行政统计语境（州及行政区）和/或警察统计语境（州及警区）。行政区与警区是独立口径；同一坐标、请求口径和边界资料版本必须产生一致结果。 | 输入是合法地点坐标和所请求口径，而非 Map 的可变状态。每个请求口径分别返回：`resolved`（名称/稳定边界标识、适用州、资料来源与版本）、`unresolved`（可解释原因及已知资料版本）或 `ambiguous`（所有候选及资料来源与版本）。若州可解析而行政区或警区不能解析，已解析州可供结果使用，但未解析层级仍必须如实标记；不把它包装为完整的地区解析。`ambiguous` 不选择任一候选；`unresolved` 不以最近、相邻、默认或名称猜测地区替代。 | 只供 opened 主应用流程的公共只读请求；不读取或写入账户资料。可读取/维护无账户的公共边界缓存，但不得改写 Map 地点、消费者分析结果或任何边界资料。缓存失效和请求时序是 Owner 内部策略，结果版本改变时旧结果不可伪装为当前版本。 |
| `GEO-002` | `GEO-001` 的生产与确定性测试 Adapter | 对给定坐标和单一地理口径读取版本化边界对象，公开零、一或多匹配以及该批边界资料的来源、版本与可读状态。 | 输入是坐标与行政区或警区口径。零匹配返回可区分的未覆盖/资料不可读等原因；一匹配返回对应边界事实；多匹配返回完整候选集，均附同一读取批次的资料来源与版本。资料版本不存在、来源无法验证或读取不可用时，不能宣称已解析。空间匹配细则在 `RISK-GEO-02` 关闭前仍待验证，但其结果必须可重复验证。 | 只读外部/镜像边界资料与无账户公共缓存；不作网络或数据写入承诺、不修改地点或下游结果。生产 Adapter 与固定样本测试 Adapter 的替换不得改变上述可观察结果语义。 |
...
| 警察统计地理解析 | [Schema Catalog：`police_districts_boundary`](../data/schema-catalog.md#公共政府镜像与边界对象)；[治安事实源](../../knowledge_base/locatemy_product/features/crime_security.md#数据边界) | Geographic Context 经 `GEO-002` 读取；Crime 只消费 `GEO-001` 结果 | 警区与行政区分开；现有对象需补足 source version 后才是完整契约证据 |
...
- **口径分离**：行政区用于收入、供水、供电、医疗、教育等地区统计；警区用于犯罪统计。任何消费者均不得将一种结果当成另一种。
- **逐层诚实**：请求的每个层级各自报告结果。州已解析不证明行政区或警区已解析；反之亦然。
...
| 请求警察统计语境 | 返回警区及其州，附来源/版本，供安全读数和警区地图使用 | 返回警区的 unresolved 或 ambiguous | 不以行政区代替警区；Crime 据此显示其既有的“暂不可用”语义 |
...
| Crime（`D13`；`SAFE-01`、`SAFE-02`） | 警区与行政区名称不同或范围不同的合法地点 | 分别请求两种口径 | 警区结果单独返回；安全分析/图层不使用行政区冒充警区 |
...
| `GEO-002` / `RISK-GEO-02` | 边界点、离岛、重叠、多边形外坐标和行政/警区不同版本样本 | 用同一固定样本重复请求两种口径 | 每例稳定地产生零、一或多匹配；多匹配完整列候选；每个结果附正确来源/版本；无附近替代 |
| 跨消费者一致性 | 两个消费者以同一地点、口径和版本并行/重复请求 | 比较结果 | 地理标识、名称、州、来源与版本一致；不共享账户资料或改写彼此分析 |

### 未关闭 Ready Gates

- [ ] **`RISK-GEO-02`（阻塞 Geographic Context 与下游 Feature Ready）**：项目负责人确认可用行政区和警区边界资料的权威来源、许可/导入版本，并用边界点、离岛、重叠、零覆盖坐标和跨版本样本证明空间匹配与多匹配语义。最迟在 Geographic Context 进入 `Ready for Development` 前关闭；证据写入风险记录而非本设计复制。
- [ ] **Schema 可用性（`RISK-SCHEMA-01` 的本 Module 部分）**：`administrative_district_boundaries` 已按 Catalog 导入并可读；`police_districts_boundary` 已兼容补足 source version；稳定公开读取路径、访问控制和实际导入审计均有证据。最迟在 Geographic Context `Ready for Development` 前关闭。
...
- [x] 行政区、州与警区的口径分离、版本保留和无推测回退已链接唯一事实源并写明应用语义。
...
| 2026-09-14 | `Draft` | 项目负责人选定行政区运行资料，并将警区正式资料交付保留为 Ready Gate 前置条件；详见 [`RISK-GEO-02`](../system/risks-and-decisions.md#risk-geo-02-资料决定与未关闭证据) | Cost、Crime、Socio-economic、Infrastructure；`GEO-001`、`GEO-002`；两类边界对象 | 项目负责人 |
