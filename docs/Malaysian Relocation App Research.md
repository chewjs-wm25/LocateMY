...
交通通勤与公共交通连通性维度：通勤时间与出行成本直接影响日常生活幸福感。公共交通网络的覆盖密度（如 Prasarana 轨交、KTMB 铁路及各州 BAS.MY 停靠站点）与实时调度状况，决定了非自驾群体的出行便利度。
...
公共交通连通性与站点密度评分引擎：基于 GTFS 静态与实时数据，计算目标居住点指定半径内的公共交通站点可达性评分与线路丰富度。
...
GTFS Realtime：提供 KTMB 等线路的车辆实时位置 Protocol Buffers 格式数据。
