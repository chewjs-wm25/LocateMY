...
LocateMY 已进入正式开发与整体精简阶段。2026-09-17 只读调查已确认生产代码存在认证、应用入口、地图、首页、隐患、设施和公共交通等部分能力；完整预算、房产与若干分析 Feature 尚未实现。各功能中的历史原型描述仅解释原型，不作为当前实现或验收完成证据。

## 项目交付边界

LocateMY 是一次性的大学项目。交付重点是学生对 Flutter，以及 SQLite、设备本地文件、键值存储和 Supabase 的数据处理能力；不以长期运营、自动化数据同步或额外安全能力为目标。2026-09-17 用户确认最小账号、在线账号记录、普通页面导航及明确的本机存储用途；删除自建会话／清理协调、离线队列、个人化适配度总分及五项评估偏好。房产回收站／恢复与档案风险快照保留，最新取舍见 [ADR 0017](../../adr/0017-minimal-account-and-online-user-records.md)。

AI 获授权编写、修改、测试和验证全部项目代码，包括 Flutter 应用行为、应用测试、Supabase migration/RLS、CLI 配置与资料导入支持；详见 [ADR 0016](../../adr/0016-ai-managed-supabase-development-environment.md)。本次项目负责人明确批准重新调整现有范围与设计，以最新确认决定和 ADR 0017 为准。当前会话交付方案与规格，不直接启动业务重构。
...
2026-09-17 用户确认保持 Supabase Free 套餐，PriceCatcher 仅导入固定 `cost-basket-v1` 的 11 个 `item_code` 对应价格，并覆盖所需 12 个月窗口；不导入其他商品价格。官方业务列和篮子定义保持不变，导入审计记录来源文件、筛选范围与实际数据日期。官方观测缺失不能因筛选或导入被补零、替换商品或伪装完整覆盖。

## 本轮分析页面精简

2026-09-17 用户确认保留指标、仅简化算法和提示，最终要求以其提供的表格为修改边界。本次采用 [UI 精简边界](ui_design_spec.md#分析页面精简边界2026-09-17)：减少技术元数据、重复说明和时效提示，不改变公式、固定半径、日期语义或缓存机制；固定快照及整体模型替代方案撤回。文档已更新，代码交给后续任务执行。
