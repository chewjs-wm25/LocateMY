...
令 `q(i)` 为该篮子版本固定的单身成年人月数量，`A(d,t)` 为地点该月有有效价格的商品集合，`E` 为可选的额外生活开销（空值为 0），`H` 与 `T` 为用户输入的住房和交通月支出。先计算核心市场观测部分：
...
满足核心市场数据质量门槛时，`ObservedSpend12` 是地点的核心市场估算月支出；住房与交通也存在时，`ScenarioSpend12` 才可展示为包含个人预案的完整月支出。否则保留 `ObservedSpend12`、缺失项目、缺失输入和覆盖率。

#### 篮子覆盖率与指数可用性

核心市场数据质量不满足时，仍展示部分篮子金额和覆盖率，但不展示地点成本指数或预算压力；不能以部分篮子金额计算个人预算压力。
...
不满足时仍显示部分篮子金额和覆盖率，但不显示地点成本指数。不能通过放大剩余项目权重或以零填补缺失来制造完整指数。

全国基准使用同一篮子数量 `q(i)` 与全国 PriceCatcher 代表价格；固定基准篮子金额为 `BaseSpend12`：

```text
CostIndex(d) = 100 × ObservedSpend12(d) / BaseSpend12
```

指数 100 表示同版核心市场篮子与全国单身成年人基准相同；大于 100 表示该核心市场篮子相对更贵，小于 100 表示相对更便宜。住房、交通和额外生活开销不进入指数。指数是辅助读数，RM/月是主结果；指数不是官方 CPI，也不是政府评级。

#### 预算压力

个人预算压力仅在 `ScenarioSpend12` 满足完整数据质量门槛、住房与交通输入已存在且用户月净收入已填写时计算；生活篮子月份不足 6 个月或平均覆盖率低于 80% 时，明确显示预算压力不可计算，不使用部分篮子金额代替。
