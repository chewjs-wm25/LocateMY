...
kind: feature-spec
capabilities: [COST-01, COST-02, COST-03, COST-04, COST-05]
...
- 支持读取、新建、选择、重命名和删除。
- 重开发预案字段统一为生活篮子调整、住房支出、交通支出、月净收入和家庭月度总收入；当前原型的预案选择和编辑仍不完整。
...
- 预算预案的编辑、保存与切换必须统一；切换后所有依赖预案的预算压力立即重算。
- 当前评估预案必须能表达可选的统一生活篮子调整（额外生活开销）、住房支出、交通支出、月净收入和家庭月度总收入；住房或交通缺失时，按单点生活成本模型显示相应的个人预案不可用状态。
- 月净收入仅用于个人预算压力；家庭月度总收入仅供社会经济的家庭收入位置读取。两者都允许缺失且不能互相替代；旧预案不猜测补齐。
- 必须保留不保存的“当前月支出”CPI 等效预算换算器，以兑现[大学提交承诺基线](../submission_commitments.md)；它只服务等效换算，预算压力仍只使用当前评估预案。
- 预案名称必须校验。
- 账户保存一份当前评估预案；它是预算压力的必要输入。
- “查看商家”与商品下钻（`COST-04`）归类为 `excluded`，本次不实现，也不显示入口。

## 已批准的预算流程规则

- 临时“当前月支出”是全国基准金额。仅当选定地点所属州和全国均有同一最新共同月份的 Headline/Overall CPI 时，才显示 `EquivalentRM = InputRM × StateHeadlineCPI / NationalHeadlineCPI`；任一输入缺失即为 unavailable，不能跨月补值。它仍只服务临时等效换算，不写入预案、预算压力。
- 删除 current 预案后，账户进入没有 current 的状态，直到用户显式选择一份仍存在的预案；不自动选择其他预案。最后一份预案也允许删除。
...
- 默认对象是单身成年人 1 人。`cost-basket-v1` 的 11 项及其月数量是固定的 PriceCatcher 核心市场篮子；不支持逐项数量或频次调整，也不宣称覆盖九类完整生活支出。
- 可空、非负的统一生活篮子调整是用户预案中的额外生活开销 RM/月；空值表示没有额外开销。它不改变篮子版本、`q(i)`、`w(i)` 或地点成本指数。
- 储蓄、保险、税费和债务还款不属于核心市场篮子；如需计入个人开销，只能作为上述额外生活开销，而不成为地点价格或指数输入。
...
行政区当月缺少商品价格时，直接从篮子中删除该商品；不以 RM 0、最近月份、州价或全国价替代。删项后的金额、指数和预算压力是明确标注为“部分篮子”的读数，不能称为完整 11 项月支出或完整生活成本指数。

#### 统一生活篮子估算月支出

令 `q(i)` 为该篮子版本固定的单身成年人月数量，`A(d,t)` 为地点该月有有效价格的商品集合，`N` 为有全国代表价的商品集合，`B(d,t) = A(d,t) ∩ N` 为可用于地点／全国同组比较的可观测子集，`E` 为可选的额外生活开销（空值为 0），`H` 与 `T` 为用户输入的住房和交通月支出。先计算核心市场观测部分：

```text
ObservedSpend(d,t) = Σ[i ∈ B(d,t)] q(i) × p(i,d,t)
```

只有 `H` 与 `T` 都已填写或明确为 RM 0 时，才形成个人预算预案：

```text
ScenarioSpend(d,t) = ObservedSpend(d,t) + E + H + T
```
...
当 `B(d,t)` 未覆盖完整 11 项或月份不足 6 个月时，`ObservedSpend12` 是部分篮子估算月支出；住房与交通也存在时，`ScenarioSpend12` 是包含个人预案的部分篮子月支出。页面必须提示资料不完整，并显示可观测项目数（/11）和月份数。

#### 数据不完整与部分篮子指数

资料不完整不阻止计算。`B(d,t)` 中每项都有同一商品的地点及全国价格，因此可保留原始缺失且不以零填补的前提下计算部分篮子指数。页面只显示可观测项目数（/11）和可用月份数，不显示百分比覆盖率。

对每个有非空 `B(d,t)` 的月份，使用完全相同商品集合的全国代表价作为分母：

```text
PartialIndex(d,t) = 100 ×
  Σ[i ∈ B(d,t)] q(i) × p(i,d,t) /
  Σ[i ∈ B(d,t)] q(i) × national_price(i)

PartialIndex12(d) = average of PartialIndex(d,t)
                    over months where B(d,t) is non-empty
```

`PartialIndex12` 只有在至少一个月份存在同组地点／全国价格时才可计算。指数 100 表示该月可观测子集与同组全国基准相同；大于 100 表示该子集相对更贵，小于 100 表示相对更便宜。住房、交通和额外生活开销不进入指数。它是辅助读数，RM/月是主结果；它不是官方 CPI、政府评级或完整 11 项生活成本指数。A/B 可以并列比较部分篮子指数，但每边都必须保留资料不完整提示，不能暗示两边观察到相同商品集合。

#### 预算压力

个人预算压力在 `ScenarioSpend12` 可计算、住房与交通输入已存在且用户月净收入已填写时计算；市场篮子资料不完整时，结果必须标为“部分篮子预算压力”。住房、交通或月净收入未填写时仍不可计算，不能把未填写当作 RM 0。
...
- 统一生活篮子的项目、数量、版本、`w(i)` 和 `BaseSpend12` 冻结规则见[统一生活篮子 v1](../cost_basket_v1.md)；该资料是本模型中产品假设的唯一事实源。
