...
- 临时“当前月支出”是全国基准金额。仅当选定地点所属州和全国均有同一最新共同月份的 Headline/Overall CPI 时，才显示 `EquivalentRM = InputRM × StateHeadlineCPI / NationalHeadlineCPI`；任一输入缺失即为 unavailable，不能跨月补值。它仍只服务临时等效换算，不写入预案、预算压力或个人化地点适配度。
...
核心市场数据质量不满足时，仍展示部分篮子金额和覆盖率，但不展示地点成本指数或预算压力；不能以部分篮子金额计算个人预算压力或其适配度成本转换分。
