...
服务日期范围、availability 与 service outcome 的判定以[公共交通功能事实源](features/transportation.md#数据处理边界)为唯一权威；解析失败或预期 feed 未取得时，站点结果可以保留已成功部分，但交通分按该唯一矩阵处理。刷新失败时依该唯一规则保留上一次成功结果；从未成功取得结果时显示“暂不可用”。
...
账户的医疗、教育和公共交通权重只影响本页单点 ICI：合法未保存滑块值可即时显示为 `unsaved preview`，但只有远端保存成功才成为跨设备的已保存权重；保存失败保留草稿/预览，用户可重试或恢复最后一次已保存值。地点 A/B 和地点摘要以三项均为中性值 5 的 ICI 为准。2026-09-17 已取消个人化地点适配度和账户五项评估偏好；三项 ICI 权重仍保留，不增加另一层账户偏好。
...
页面同时显示地点行政区、综合指数等级和缺失分项；人口年份差异简短表达，重复期间说明可合并，辅助信息按 [UI 精简边界](ui_design_spec.md#分析页面精简边界2026-09-17) 呈现。该指数不能比较服务质量，也不能证明某项服务在用户需要的时间可用。

部分交通资料仍可评分；同日期参照网格缺失时只使用距离项并归一化权重，明确提示“仅距离项”。详见公共交通功能事实源的部分交通分规则。ICI 复用该读数并保留部分交通输入提示。
