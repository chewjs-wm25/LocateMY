...
每个 feed 保存 `feed_captured_at`，页面显示 feed 采集时间和本次刷新时间。可读取但长时间未刷新的 feed 仍可使用，并标记“可能过期”；解析失败或预期 feed 未取得时，站点结果可以保留已成功部分，但交通分标记 `incomplete`（“资料不完整，无法生成完整交通分”）。已取得站点但分析日期没有有效路线时标记 `no_active_routes`（“有站点，但分析日期没有有效服务路线”）。刷新失败时保留上一次成功结果并标记可能过期；从未成功取得结果时显示“暂不可用”。
