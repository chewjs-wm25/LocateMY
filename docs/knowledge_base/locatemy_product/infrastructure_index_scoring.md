...
账户的医疗、教育和公共交通权重只影响本页单点 ICI：合法未保存滑块值可即时显示为 `unsaved preview`，但只有远端保存成功才成为跨设备的已保存权重；保存失败保留草稿/预览，用户可重试或恢复最后一次已保存值。地点 A/B、地点摘要和个人化地点适配度一律以三项均为中性值 5 的 ICI 为准。账户五项评估偏好只用于适配度总分，不再叠加到 ICI 内部。
