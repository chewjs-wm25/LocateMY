...
2026-09-17 取舍说明：以下矩阵仅描述历史原型。个人化地点适配度与账户五项偏好已由 ADR 0017 取消；相关历史公式／缺口不再是当前开发要求。
...
| 治安与犯罪单点 | 州级 0–100 安全指数状态、最近五年案件趋势、类别筛选 | SAFE-01/03 | `features/crime_security.md` | 已记录／原型缺口：责任边界、州级口径和缺失规则齐全；当前安全卡和趋势全为硬编码示例，不依赖 selected 地点 | `analysis_page.dart` `_safetyPage()`、`_crimeSeries` |
...
| 地点摘要安全 | 0–100 州级安全指数；缺失显示 `—` | `crime_security.md`：案件量 `log(1+x)`，同年度统计州百分位，assault/property 按 60%/40%反向转换 | `crime_district`；地点所属统计州；年度，使用最新完整年度 | 统计州无法匹配或两个类别均无数据时不可用；不是人口标准化犯罪率、官方评级或隐患分 | 已记录／原型缺口；摘要固定显示“安全指数暂不可用”，尚未接入州和年度服务 |
...
| 治安与犯罪 | 州级安全指数显示 `—/100`；趋势案件数，筛选只影响趋势；代码固定 2020–2024 五年，不随地点变化 | 总览与详情均为不可用/待接入的硬编码状态，不计算差异 | `crime_security.md`；`crime_district`、统计州、年度已定罪案件数；不是人口率 | 已记录／原型缺口。2024 示例年份不应被解读为当前官方数据；比较缺少逐地点来源、日期和可比性字段 |
...
2. 单点安全页的趋势和州名是全局固定 fixture；其中趋势包含 2024，而 canonical 犯罪数据说明当前官方页面截至 2023。页面已标为示例，不能据此写成官方最新年份。
