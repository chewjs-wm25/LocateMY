...
- 有效缓存可直接显示；查询／缓存时间保留于内部，不展示给用户。
- 用户主动刷新时绕过缓存并重新查询。
- 查询失败时，仍在 24 小时内的缓存可继续显示，刷新失败保留简短错误与重试，不显示缓存时间或较旧警告；没有可用缓存时显示“资料暂不可用”。
- 页面技术元数据与说明按 [UI 精简边界](ui_design_spec.md#分析页面精简边界2026-09-17) 呈现；设施称为“已收录设施”，取消独立来源／查询时间行，重复覆盖说明合并。
