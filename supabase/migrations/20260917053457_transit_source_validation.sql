-- A usable feed must carry verifiable capture and service-date boundaries.
alter table public.gtfs_feed_snapshots add constraint transit_usable_metadata
 check(parse_status <> 'usable' or (captured_at is not null and service_start is not null
 and service_end is not null and service_start <= service_end));
...
('gtfs_static_mybas_kuching','https://api.data.gov.my/gtfs-static/mybas-kuching')
 ...
 select e.feed_id,coalesce(f.source_id,e.feed_id) as source_id,
 coalesce(f.source_url,e.source_url) as source_url,f.captured_at,
 coalesce(f.failure_reason,case when f.feed_id is null then 'Expected feed snapshot missing' end) as failure_reason,
 case when f.feed_id is null or parse_status='missing' then 'missing'
