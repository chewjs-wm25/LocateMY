...
void main() {
  group('FacilityRepository.getNearbyFacilities', () {
    test('HTTP 200：解析 node/way、按距离过滤与升序、无名要素友好回退', () async {
      ...
      expect(
        requested.single.host,
        FacilityRepository.overpassEndpoints.first
            .replaceFirst('https://', '')
            .split('/')
            .first,
      );
      ...
      expect(
        results.every(
          (f) => f.distance <= FacilityRepository.defaultRadiusMeters,
        ),
        isTrue,
      );
    ...
    test('全部端点失败（非 200）→ 抛出 FacilityLoadException 并含各端点原因', () async {
      ...
      await expectLater(
        ...
        throwsA(
          isA<FacilityLoadException>().having(
            (e) => e.message,
            'message',
            contains('HTTP 500'),
          ),
    ...
    test('去重：node 与 way 同一设施（同类型/名称/约同坐标）只保留一条', () async {
      ...
      final repo = FacilityRepository(
        client: MockClient(
          (request) async => http.Response(duplicatedBody, 200),
        ),
      );
  ...
  group('FacilityRepository.overpassEndpoints', () {
    test('至少配置 2 个端点用于回退', () {
      expect(
        FacilityRepository.overpassEndpoints.length,
