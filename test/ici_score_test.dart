...
void main() {
  group('computeIciScore 公平口径', () {
    ...
    test('完全无数据 → null', () {
      ...
      expect(computeIciScore({'water': null, 'power': null, 'healthcare': null, 'education': null, 'transit': null}, transitApplicable: false), isNull);
    });

    test('分数越界被收敛到 0-100', () {
      final s = {'water': 150, 'power': -10, 'healthcare': 100, 'education': 100, 'transit': 100};
      final v = computeIciScore(s, transitApplicable: true)!;
      expect(v, inInclusiveRange(0, 100));
    });

    test('自定义权重生效', () {
      final s = {'water': 100, 'power': 100, 'healthcare': 100, 'education': 100, 'transit': 100};
      final heavyHealth = computeIciScore(s, transitApplicable: true, wHealth: 0.6, wWater: 0.1, wPower: 0.1, wEdu: 0.1, wTransit: 0.1);
      expect(heavyHealth, closeTo(100.0, 1e-9));
      // 医疗差(0分)但权重高达 0.6 时,整体被明显拉低: 0.6*0 + 0.1*100*3 + 0.1*100 = 40。
      final s2 = {'water': 100, 'power': 100, 'healthcare': 0, 'education': 100, 'transit': 100};
      final heavyHealth2 = computeIciScore(s2, transitApplicable: true, wHealth: 0.6, wWater: 0.1, wPower: 0.1, wEdu: 0.1, wTransit: 0.1);
  ...
  group('missingSourceItems', () {
    ...
    test('缺医疗与教育(有坐标)→ 两者被列出', () {
      final s = {'water': 100, 'power': 100, 'healthcare': null, 'education': null, 'transit': 100};
      expect(missingSourceItems(s, transitApplicable: true), containsAll(['healthcare', 'education']));
    });
    ...
    test('transit 可评估但无数据 → 计入缺失', () {
      final s = {'water': 100, 'power': 100, 'healthcare': 100, 'education': 100, 'transit': null};
      expect(missingSourceItems(s, transitApplicable: true), contains('transit'));
    });
  });
}
