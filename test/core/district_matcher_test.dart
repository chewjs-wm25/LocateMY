...
void main() {
  group('DistrictMatcher.matches 跨源县名匹配', () {
    ...
    test('括号后缀(医院口径)', () {
      expect(
        DistrictMatcher.matches('Petaling', 'Petaling (Subang Jaya)'),
        isTrue,
      );
      expect(
        DistrictMatcher.matches('Timur Laut', 'Timur Laut (Georgetown)'),
        isTrue,
      );
      expect(DistrictMatcher.matches('Gombak', 'Gombak (Rawang)'), isTrue);
      expect(
        DistrictMatcher.matches('Larut & Matang', 'Larut & Matang (Taiping)'),
        isTrue,
      );
    });
    ...
    test('并县条目:行名前缀或组成县命中', () {
      ...
      expect(
        DistrictMatcher.matches('Bandar Baharu', 'Kulim/Bandar Baharu'),
        isTrue,
      );
    });
    ...
    test('缩写别名 S.P.* → Seberang Perai *', () {
      expect(
        DistrictMatcher.matches('S.P. Selatan', 'Seberang Perai Selatan'),
        isTrue,
      );
      expect(
        DistrictMatcher.matches('S.P.Tengah', 'Seberang Perai Tengah'),
        isTrue,
      );
      expect(
        DistrictMatcher.matches('S.P.Utara', 'Seberang Perai Utara'),
        isTrue,
      );
    });
    ...
    test('Ulu/Hulu 用字差异', () {
      ...
      expect(
        DistrictMatcher.matches('Hulu Langat', 'Hulu Langat (Bangi)'),
        isTrue,
      );
    });
    ...
    test('联邦直辖区 W.P. 前缀差异', () {
      expect(
        DistrictMatcher.matches('Kuala Lumpur', 'W.P. Kuala Lumpur'),
        isTrue,
      );
      expect(
        DistrictMatcher.matches('W.P. Kuala Lumpur', 'Kuala Lumpur'),
        isTrue,
      );
    ...
    test('连接词 dan/and 差异', () {
      expect(
        DistrictMatcher.matches('Larut dan Matang', 'Larut & Matang (Taiping)'),
        isTrue,
      );
      expect(
        DistrictMatcher.matches('Larut & Matang', 'Larut dan Matang'),
        isTrue,
      );
    });
